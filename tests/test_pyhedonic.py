...
VALUATIONS2 = np.array([[0, 9, 9, 4], [9, 0, 1, 7], [9, 1, 0, 7], [4, 7, 7, 0]])
...
def test_HedonicGame_coalition_structures_1():
    ...
    compare_iterable_nparray(
        GAME1_FRAC.coalition_structures(cs_size=2), [[0, 0, 1], [0, 1, 0], [0, 1, 1]]
    )
    compare_iterable_nparray(
        GAME1_FRAC_K2.coalition_structures(cs_size=2), [[0, 0, 1], [0, 1, 0], [0, 1, 1]]
    )
...
def test_HedonicGame_prices_for_2SSFHG():
    ...
    assert pr.poa == Fraction(2, 1)
    assert pr.pos == Fraction(1, 1)
